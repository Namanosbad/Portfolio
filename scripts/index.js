...
const translations = {
  pt: {
    ...
    'projects.ETrocas.description': 'O ETrocas é uma plataforma de trocas criada para conectar pessoas que querem desapegar de itens em bom estado e encontrar novas oportunidades de uso para eles. O projeto prioriza uma experiência simples e intuitiva, incentivando o consumo consciente e a economia colaborativa por meio de negociações diretas entre usuários.',
    'projects.ETrocas.link': 'Acessar projeto →',
    'projects.invoiceAPI.title': 'InvoiceAPI 💸📄',
    'projects.invoiceAPI.description': 'InvoiceAPI é uma solução para criação e gerenciamento de faturas, projetada para automatizar cálculos e garantir a integridade dos dados financeiros. Através de um fluxo de operações padronizado, a API assegura que qualquer alteração nos itens seja refletida instantaneamente no valor total, eliminando inconsistências manuais. O projeto prioriza a previsibilidade e a sincronização de dados, oferecendo controle total sobre o status e os itens de cada fatura em um ambiente estruturado e confiável.',
    'projects.invoiceAPI.link': 'Acessar projeto →',
  ...
  en: {
    ...
    'projects.ETrocas.description': 'ETrocas is a swapping platform built to connect people who want to pass on items in good condition and give them a new purpose. The project focuses on a simple and intuitive experience, encouraging conscious consumption and collaborative economy through direct user-to-user exchanges.',
    'projects.ETrocas.link': 'View project →',
    'projects.invoiceAPI.title': 'InvoiceAPI 💸📄',
    'projects.invoiceAPI.description': 'InvoiceAPI is a solution for creating and managing invoices, designed to automate calculations and ensure the integrity of financial data. Through a standardized operations flow, the API guarantees that any change in items is instantly reflected in the total amount, eliminating manual inconsistencies. The project prioritizes predictability and data synchronization, offering full control over the status and items of each invoice in a structured and reliable environment.',
    'projects.invoiceAPI.link': 'View project →',
